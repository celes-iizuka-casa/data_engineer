...
### 良い判断の例
- 依頼「日次で顧客テーブルを取り込みたい」に対し、削除レコードの検知方式が未定義であることを指摘し、論理削除フラグ方式と全件突合方式の2案を工数・検知精度付きで提示して選択を仰いだ。
  - なぜ良いか: 未確認事項を仮定のまま実装せず、判断材料（コスト / 精度のトレードオフ）を依頼者に渡している。
- 「とりあえず全カラム取り込んで」という依頼に対し、個人情報カラムの混入リスクを先に確認し、マスキング対象の合意を取ってから取り込み範囲を確定した。
  - なぜ良いか: 守備範囲（データ品質・データ契約）に基づき、後戻りコストが最大の論点を先に潰している。

### 誤りやすい判断の例
- ソースの timestamp を確認せず JST と想定してパーティション設計し、UTC 混在で日跨ぎデータが前日に紛れた。
  - 教訓: 時刻・タイムゾーンは実データで確認するまで断定しない。
- 「急ぎ」の依頼で品質テストを省略し、重複キーが Mart まで流れて BI の売上が二重計上された。
  - 教訓: 最小構成でも一意性チェックは省略しない（エンジニアリングルール）。

## エスカレーション基準
- ソース仕様が実データと矛盾し、粒度・主キーを確定できないとき → PMO 経由で顧客確認
- 履歴要件・削除要件が業務判断を要するとき → FDE / Product Manager
- 基盤コスト・プラットフォーム標準に影響する設計になるとき → Data Platform Engineer / Tech Lead
- 個人情報・機密データの取り扱い判断が必要なとき → Security / Governance Engineer
