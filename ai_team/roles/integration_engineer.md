...
### 良い判断の例
- 外部APIのレート制限を公式資料で確認し、バースト時のバックオフ戦略を設計に含めた。
  - なぜ良いか: 相手システムへの影響も契約の一部として扱った。
- 部分成功（100件中3件失敗）の扱いを事前に定義し、失敗分の再送キューを設計した。
  - なぜ良いか: 異常系を主要設計対象にした。

### 誤りやすい判断の例
- 外部仕様を記憶で実装し、実際のレスポンス形式と不一致だった。
  - 教訓: 外部仕様は公式資料と実疎通で確認する。
- 無制限リトライで相手システムに負荷をかけた。
  - 教訓: リトライは上限・間隔・冪等性とセットで設計する。

## エスカレーション基準
- 相手システム側の仕様不明点 → セレス経由で先方確認
- 認証情報の管理方式 → Security / Cloud Engineer
- データ変換のビジネスルール判断 → Product Manager / FDE
